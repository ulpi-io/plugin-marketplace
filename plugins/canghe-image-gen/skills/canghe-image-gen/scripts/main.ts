...
Options:
  ...
  --provider google|openai|dashscope|canghe  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
  --ar <ratio>              Aspect ratio (e.g., 16:9, 1:1, 4:3)
  --size <WxH>              Size (e.g., 1024x1024)
  --quality normal|2k       Quality preset (default: 2k)
  --imageSize 1K|2K|4K      Image size for Google (default: from quality)
  --ref <files...>          Reference images (Google multimodal, OpenAI edits, or Canghe image_url)
...
Environment variables:
  ...
  CANGHE_IMAGE_MODEL        Default Canghe model (gemini-3-pro-image-preview)
  OPENAI_BASE_URL           Custom OpenAI endpoint
  GOOGLE_BASE_URL           Custom Google endpoint
  DASHSCOPE_BASE_URL        Custom DashScope endpoint
  CANGHE_BASE_URL           Custom Canghe endpoint (default: https://api.canghe.ai/v1)

Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.canghe-skills/.env > ~/.canghe-skills/.env`);
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "canghe") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".canghe-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".canghe-skills", ".env"));
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, canghe: null };
        currentKey = "default_model";
      } else if (
        currentKey === "default_model" &&
        (key === "google" || key === "openai" || key === "dashscope" || key === "canghe")
      ) {
        const cleaned = value.replace(/['"]/g, "");
        const providerKey = key as keyof NonNullable<ExtendConfig["default_model"]>;
        config.default_model[providerKey] = cleaned === "null" ? null : cleaned;
      }
    }
  }
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(cwd, ".canghe-skills", "canghe-image-gen", "EXTEND.md"),
    path.join(home, ".canghe-skills", "canghe-image-gen", "EXTEND.md"),
  ];
...
function detectProvider(args: CliArgs): Provider {
  if (
    ...
    args.provider !== "canghe"
  ) {
    throw new Error(
      "Reference images require a ref-capable provider. Use --provider google (Gemini multimodal), --provider openai (GPT Image edits), or --provider canghe."
    );
  }
  ...
  const hasCanghe = !!process.env.CANGHE_API_KEY;

  if (args.referenceImages.length > 0) {
    if (hasGoogle) return "google";
    if (hasOpenai) return "openai";
    if (hasCanghe) return "canghe";
    throw new Error(
      "Reference images require Google, OpenAI, or Canghe. Set GOOGLE_API_KEY/GEMINI_API_KEY, OPENAI_API_KEY, or CANGHE_API_KEY, or remove --ref."
    );
  }

  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasCanghe && "canghe"].filter(
    Boolean
  ) as Provider[];
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or CANGHE_API_KEY.\n" +
      "Create ~/.canghe-skills/.env or <cwd>/.canghe-skills/.env with your keys."
  );
}
...
async function validateReferenceImages(referenceImages: string[], provider: Provider): Promise<void> {
  for (const refPath of referenceImages) {
    if (provider === "canghe" && isHttpUrl(refPath)) {
      continue;
    }
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "canghe") {
    return (await import("./providers/canghe")) as ProviderModule;
  }
...
async function main(): Promise<void> {
  ...
  if (mergedArgs.referenceImages.length > 0) {
    await validateReferenceImages(mergedArgs.referenceImages, provider);
  }
  ...
  if (!model && extendConfig.default_model) {
    ...
    if (provider === "canghe") model = extendConfig.default_model.canghe ?? null;
  }
