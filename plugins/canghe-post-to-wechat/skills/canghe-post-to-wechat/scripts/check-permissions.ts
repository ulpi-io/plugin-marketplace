...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.canghe-skills', '.env');
  const userEnv = path.join(os.homedir(), '.canghe-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== canghe-post-to-wechat: Permission & Environment Check ===\n');
