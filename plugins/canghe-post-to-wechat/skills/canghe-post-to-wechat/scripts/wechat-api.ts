...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".canghe-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".canghe-skills", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .canghe-skills/.env file."
    );
  }
