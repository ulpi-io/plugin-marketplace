...
class_name RomanceDateEventSystem
extends Node
...
func evaluate_date(character_id: String, location_data: Dictionary, choice_modifiers: int = 0) -> Outcome:
	var stats: Dictionary = RomanceAffectionManager.get_stats(character_id)
...
func _apply_outcome_rewards(char_id: String, outcome: Outcome) -> void:
	match outcome:
		Outcome.PERFECT:
			RomanceAffectionManager.add_stat(char_id, RomanceAffectionManager.RelationStat.ATTRACTION, 10)
			RomanceAffectionManager.add_stat(char_id, RomanceAffectionManager.RelationStat.TRUST, 5)
		Outcome.SUCCESS:
			RomanceAffectionManager.add_stat(char_id, RomanceAffectionManager.RelationStat.ATTRACTION, 5)
		Outcome.DISASTER:
			RomanceAffectionManager.add_stat(char_id, RomanceAffectionManager.RelationStat.TRUST, -5)
			RomanceAffectionManager.add_stat(char_id, RomanceAffectionManager.RelationStat.ATTRACTION, -2)
