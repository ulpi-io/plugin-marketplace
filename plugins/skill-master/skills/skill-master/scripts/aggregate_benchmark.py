...
Usage:
    python -m scripts.aggregate_benchmark <benchmark_dir> --skill-name my-skill

The script supports two directory layouts:

    Workspace layout (from skill iterations):
    <benchmark_dir>/
    └── eval-N/
        ├── with_skill/
        │   ├── run-1/grading.json
...
def generate_markdown(benchmark: dict) -> str:
    ...
    def fmt(stat, pct=False):
        if not stat: return "—"
        m = f"{stat.get('mean',0)*100:.0f}%" if pct else f"{stat.get('mean',0):.1f}"
        s = f"{stat.get('stddev',0)*100:.0f}%" if pct else f"{stat.get('stddev',0):.1f}"
        return f"{m} ± {s}"

    lines.append(f"| Pass Rate | {fmt(a.get('pass_rate'), True)} | {fmt(b.get('pass_rate'), True)} | {delta.get('pass_rate', '—')} |")
    lines.append(f"| Time | {fmt(a.get('time_seconds'))}s | {fmt(b.get('time_seconds'))}s | {delta.get('time_seconds', '—')}s |")
    lines.append(f"| Tokens | {fmt(a.get('tokens'))} | {fmt(b.get('tokens'))} | {delta.get('tokens', '—')} |")
